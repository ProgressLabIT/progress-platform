...
- **Plugins**: [webapps/main/src/plugins/](webapps/main/src/plugins/) — `createLinkedText`, `createLinkedImage`, `createLinkedBarcodes` (factory functions taking `customFields`). Link configuration is form-render format with `props.options` for selects and boolean `hidden` from `activeSchema`.
