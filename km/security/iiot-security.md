...
4. **Device identity unification + reactive-event trust model** (incoming — [ADR-0015](../decisions/0015-device-consumer-identity-model.md) / [ADR-0016](../decisions/0016-reactive-event-automation-contract.md), both `discussion`). Two principles being formalized: (a) **Device ≠ Asset** — a `DEVICE` is an auth/identity type (NFC readers, kiosks, edge nodes), distinct from the OT `Asset`/`AssetClass` domain model; one registry links them only when both exist. (b) **The channel proves origin, not payload truth** — `DEVICE` clients publish only their own channel, but subscribers of identity/command (inbound-context) channels must authenticate the *claim*, not trust the channel. Extends the §3.3 tightest-grant intent to the login/routing use cases.
