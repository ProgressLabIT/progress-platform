...
3. **Device token unification (ADR-0015, `discussion`).** The enrollment service becomes the single front door for *all* non-human identity — one `Device` registry record drives both the NATS user JWT minted here **and** a Progress-API device token (`ConsumerType.DEVICE`, `tokens.md`). Identity is unified at the registry; enforcement stays per-plane (NATS authz in JWT claims here; API authz in the `Token` collection — never cross them). This resolves §7.1 in favour of device-generated seeds, and treats NATS **auth callout** as the deferred upgrade over static-mint (one credential presented everywhere, permissions synthesized at connect). See [ADR-0015](../decisions/0015-device-consumer-identity-model.md). Note the revocation RTO asymmetry: the API device token dies instantly; the NATS credential lags the resolver `interval`.
