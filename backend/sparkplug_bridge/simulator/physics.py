"""Numeric helpers backing the ADR-0007 simulator dynamics.

All randomness goes through a single `random.Random` instance so demo
rehearsals are reproducible (per ADR-0007 § Reset semantics — `make demo-reset`
must produce the same timeline).
"""
import math
import random
...
def linear_ramp(start_value: float, slope_per_second: float, elapsed_sec: float) -> float:
    """Deterministic linear ramp used for the pump3.Vibration anomaly window.
...
def now_ms() -> int:
    """UTC milliseconds — matches the `timestamp` field expected by pysparkplug."""
