...
class ProgressOverrideRequested(BaseAdmin):
  ...
  def apply(self):
    ...
    if self.info.quantity_change > 0:
      if self.info.quantity_change > free_wip_qt_upstream and not self.job.first_phase:
      ...
      if self.info.should_adjust_duration:
        ...
        quantity_ratio = self.info.quantity_change / self.new_job_qt_completed
      ...
      if abs(self.info.quantity_change) > free_wip_qt_downstream and not self.job.last_phase:
