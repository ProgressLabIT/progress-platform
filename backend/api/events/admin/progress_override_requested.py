...
class ProgressOverrideRequested(BaseAdmin):
  ...
  def apply(self):
    ...
    self._setup_progress_update()
    self._handle_job_status()  # Called once for both paths
    ...
    if self.info.quantity_change > 0:
        ...
        if self.info.should_adjust_duration:
            new_batch_key, batch_value = self._create_forced_traceability_records(self.info.quantity_change)
        else:
            ...
            quantity_ratio = self.info.quantity_change / self.info.new_job_qt_completed
            new_batch_key, batch_value = self._create_forced_traceability_records(
                quantity=self.info.quantity_change,
                duration=self.total_duration * quantity_ratio
            )
        ...
        if abs(self.info.quantity_change) > self.free_wip_qt_downstream and not self.job.last_phase:
        ...
        if remaining_qt_to_remove < 0:
            ...
            duration_to_preserve_total = self.total_duration * quantity_ratio
            new_batch_key, batch_value = self._create_forced_traceability_records(
    ...
    if self.job.stage == WorkStatus.CLOSED and self.info.quantity_change < 0:
  ...
  def _setup_progress_update(self):
    """
    Sets up the initial data needed for the progress update.
    This includes:
    1. Calculating new progress percentage
    2. Preparing the job update dictionary
    3. Retrieving and storing WIP data
    4. Validating quantity changes

    Raises:
        ValueError: If there's no quantity change or if new quantity exceeds planned
        WipNotAvailableError: If insufficient WIP is available for the operation
    """
    # Calculate new progress and prepare job update
    ...
    # Calculate and store WIP quantities
    ...
    # Calculate and validate quantity change
