...
class MessagePosted(BaseCollaboration):
  ...
  def set_model(self, base_model: EventModel):
    self.event_data = MessagePostedModel(**base_model.model_dump())

  def apply(self):
      issue_key = self.event_data.message_data.recipient.split('/')[1]
      self.event_data.issue_data = dict(_key=issue_key)
      message_data = self.event_data.message_data.dict(by_alias=True, exclude={'_key', '_id', '_rev'})
