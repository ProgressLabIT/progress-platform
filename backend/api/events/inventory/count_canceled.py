...
class CountCanceledEvent(BaseEvent):
  class InfoModel(EventInfoModel):
    count_key: str
    inventory_keys: list[str] | None = None
  ...
  def apply(self):
    ...
    # Handle inventory unlocking if inventory_keys are provided
    inventory_records = []
    if self.info.inventory_keys and len(self.info.inventory_keys) > 0:
    ...
    if inventory_records:
      ...
      self.response = dict(
        message=f"Count record canceled (no inventory records to unlock)",
        count_record_key=self.info.count_key
      )
