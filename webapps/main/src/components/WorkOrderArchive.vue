      ...
      <template #top>
        <div class="row full-width justify-between text-low q-mb-md">
            ...
            >
              <template #append>
                <q-icon name="mdi-calendar" size="xs" class="cursor-pointer">
                  ...
                  >
                    <q-date v-model="time_start_from" mask="YYYY-MM-DD" minimal>
              ...
              <template #append>
                <q-icon name="mdi-calendar" size="xs" class="cursor-pointer">
                  <q-popup-proxy
                    ...
                    transition-hide="scale"
                  >
                    <q-date v-model="time_start_to" mask="YYYY-MM-DD" minimal>
                      <div class="row items-center justify-end">
                        <q-btn
                          v-close-popup
                          label="Close"
              ...
              <template #append>
                <q-icon name="mdi-calendar" size="xs" class="cursor-pointer">
                  <q-popup-proxy
                    ...
                    transition-hide="scale"
                  >
                    <q-date v-model="time_end_from" mask="YYYY-MM-DD" minimal>
                      <div class="row items-center justify-end">
                        <q-btn
                          v-close-popup
                          label="Close"
              ...
              <template #append>
                <q-icon name="mdi-calendar" size="xs" class="cursor-pointer">
                  <q-popup-proxy
                    ...
                    transition-hide="scale"
                  >
                    <q-date v-model="time_end_to" mask="YYYY-MM-DD" minimal>
                      <div class="row items-center justify-end">
                        <q-btn
                          v-close-popup
                          label="Close"
          ...
          <div class="row q-gutter-md items-center">
            <div class="smaller">{{ wo_list.length }} Record (Max 100)</div>
            <q-btn color="theme-blue" :label="$t('export')" @click="export_csv">
            </q-btn>
          </div>
        </div>
      </template>
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          align: 'right',
          label:
            this.$t('performance.processing_time.medium').toUpperCase() +
            ' (TOT)',
        },
        {
        ...
        },
        {
          ...
          align: 'right',
          label:
            this.$t('performance.processing_cost.medium').toUpperCase() +
            ' (TOT)',
        },
        {
  ...
  methods: {
          ...
          setTimeout(() => {
            this.wo_list = resp.data.map((wo) => {
              return {
                ...
                unit_processing_time: this.getHumanDuration(
                  wo.processing_time / wo.qt_completed,
                ),
                processing_cost: wo.processing_cost.toFixed(2),
                unit_processing_cost: (
                  wo.processing_cost / wo.qt_completed
                ).toFixed(2),
              };
            });
            this.loading = false;
    ...
    getHumanDate(iso_string) {
      ...
      return this.$capitalize(
        this.$formatDateTime(iso_string, this.$i18n.locale, config),
      );
    },
