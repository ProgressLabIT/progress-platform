export function durationFromMillisec(
  millisec,
  {
    showDays = false,
    precision = 's',
  ...
  const hours = showDays
    ? Math.floor((abs % millisecondsPer.day) / millisecondsPer.hour)
    : Math.floor(abs / millisecondsPer.hour);
  const hoursTrue = hours >= 1;
  const showHours = ['h', 'm', 's', 'ms'].includes(precision);
  const hoursString = hoursTrue && showHours ? hours + 'h ' : '';

  const minutes = Math.floor(
    (abs % millisecondsPer.hour) / millisecondsPer.minute,
  );
  ...
  const seconds = Math.floor(
    (abs % millisecondsPer.minute) / millisecondsPer.second,
  );
