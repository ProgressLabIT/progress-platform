...
const traceability = {
  ...
  actions: {
    ...
    async saveSerialLinks({ dispatch, state }) {
      const batchLinks = state.bom_serials
        .filter(l => l.batch_key === state.current_batch_data._key)
        .map(i => ({
          ...i,
          _key: i.link_key,
      ...
      return new Promise((resolve, reject) => {
        api.put(`/batch/${state.current_batch_data._key}/serial-temp-links`, batchLinks)
        .then(async () => {
