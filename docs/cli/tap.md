...
> Subscribes to a NATS subject pattern and pretty-prints incoming messages to stdout. Acts as a generic `nats sub` wrapper with subject-shape awareness so Sparkplug raw frames (`spBv1//0.>`) and Progress notification subjects (`progress.notification.>`) render with the metadata the operator actually needs. Diagnostic-only — does not modify state.
...
- When verifying that a freshly-deployed event publishes to its expected NATS subject after the post-commit fan-out.
...
- When inspecting raw Sparkplug B frames the bridge consumes — pattern `'spBv1//0.>'` per ADR-0008's NATS-MQTT subject mapping.
...
`<pattern>` is a NATS subject filter (wildcards `*` and `>` supported). Quote the pattern in shells that interpret `>` as redirection.

## Options

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--server` / `-s` | `TEXT` | `nats://localhost:4222` | NATS broker URL. Defaults to the local broker exposed by `deploy/compose/base.yaml`. |
| `--credentials` / `-c` | `PATH` | _(none)_ | Path to a NATS credentials file (`.creds`) for authenticated brokers. |
...
| `--queue` | `TEXT` | _(none)_ | Subscribe as part of a NATS queue group (load-balanced delivery across taps). |
...
- [Integration touchpoints](/admins/integration) — NATS subject taxonomy and Sparkplug subject mapping
